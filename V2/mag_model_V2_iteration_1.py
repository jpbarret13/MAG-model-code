...
class CustomModel(tf.keras.Model):
    def train_step(self, inputs):
        ...
        paper_titles = old_features[0].to_tensor(shape=[None, 64])
...
def get_dataset(path, data_type='train'):
    ...
    raw_dataset = tf.data.TFRecordDataset(tfrecords[:12], num_parallel_reads=AUTO)
...
with mirrored_strategy.scope():
    ...
    title_position_embeddings = tf.keras.layers.Embedding(input_dim=64,
                                                          output_dim=int(args.emb_size),
                                                          weights=[get_pos_encoding_matrix(64, int(args.emb_size))],
